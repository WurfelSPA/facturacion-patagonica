 ...
 *   → crea sesión resumible, devuelve { uploadUrl }
 *   El frontend sube el archivo directo a Google (sin pasar por Vercel)
 *
 * POST { action:"upload", base64, filename, mimeType, folderId }
 *   → sube archivo pequeño via Service Account (para ZIPs, etc.)
 */

export const config = { api: { bodyParser: { sizeLimit: '20mb' }, responseLimit: '20mb' } };
...
async function getSAToken() {
  ...
  if (!data.access_token) throw new Error("SA token: " + JSON.stringify(data));
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── Acción: obtener URL de subida resumible (para archivos grandes) ───
    ...
    // ── Acción: subida directa base64 (para archivos pequeños como ZIPs) ─
    ...
    if (!uploadRes.ok) { const e = await uploadRes.text(); return res.status(502).json({ error: `Drive upload ${uploadRes.status}: ${e.slice(0,200)}` }); }
