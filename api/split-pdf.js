...
function splitPDFPages(buf) {
  ...
  for (const pageNum of pageNums) {
    ...
    // Escribir Pages object — propagar /Resources heredados si la página no los declara
    // explícitamente. Sin esto, PDFs cuyas páginas heredan /Resources del padre (ej. junio 2026)
    // generan split-PDFs con fuentes incluidas pero desconectadas → caracteres garbled.
    offsets[pagesNum] = newPdf.length;
    {
      const pageHasOwnRes = /\/Resources\s*(<<|\d+\s+0\s+R)/.test(pageObj);
      let inheritedRes = "";
      if (!pageHasOwnRes) {
        const parentM = pageObj.match(/\/Parent\s+(\d+)\s+0\s+R/);
        if (parentM) {
          const parentDict = getObj(parseInt(parentM[1]));
          const resIdx = parentDict.indexOf("/Resources");
          if (resIdx >= 0) {
            const after = parentDict.slice(resIdx + "/Resources".length).replace(/^\s+/, "");
            if (after.startsWith("<<")) {
              // Dict inline: extraer << >> balanceado
              let depth = 0, i = 0;
              for (; i < after.length - 1; i++) {
                if (after[i]==="<"&&after[i+1]==="<"){depth++;i++;}
                else if (after[i]===">"&&after[i+1]===">"){depth--;i++;if(depth===0)break;}
              }
              const dictStr = after.slice(0, i + 1);
              const remapped = dictStr.replace(/(\d+)\s+0\s+R/g,(m,n)=>{
                const mp=mapping[parseInt(n)];return mp?`${mp} 0 R`:m;
              });
              inheritedRes = ` /Resources ${remapped}`;
            } else {
              // Referencia indirecta: /Resources N 0 R
              const refM = after.match(/^(\d+)\s+0\s+R/);
              if (refM) {
                const mp = mapping[parseInt(refM[1])];
                if (mp) inheritedRes = ` /Resources ${mp} 0 R`;
              }
            }
          }
        }
      }
      newPdf += `${pagesNum} 0 obj\n<< /Type /Pages /Kids [${pageNewNum} 0 R] /Count 1${inheritedRes} >>\nendobj\n`;
    }
